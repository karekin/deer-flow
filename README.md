...
CloudMold deployments may place `business-taxonomy.json` beside the public skill packages. Authenticated callers can read the effective runtime projection through `GET /api/skills/business-catalog`, grouped as business unit → business domain → role → Skill, and retrieve an individual public/integration `SKILL.md` through `GET /api/skills/business-catalog/{skill_name}`. Both responses include SHA-256 digests and `ETag` headers. The detail endpoint never returns host paths and deliberately excludes custom and legacy skill content; CloudMold remains responsible for tenant authorization before forwarding this read-only view to its operations console.
