...
The Agents workspace can install the managed `workflow-steward` template without hand-editing agent YAML or workflow JSON. The template uses `workflow_definition_get` to retrieve and locally re-verify signed active definitions, `workflow_observation_get` to read fixed tenant-scoped managed-run/model-observation endpoints, `workflow_manage` to create the immutable local proposal bundle, and `workflow_proposal_status` / `workflow_proposal_submit` to register that bundle as a CloudMold candidate when the proposal registry API is available; governed web search remains optional. Managed-run reads use the SkillTask `skill_id` namespace, while model-observation reads require the distinct AI Operations `model_workflow_id` namespace. Registry reads reject redirects, ambient proxy credentials, unbounded pages, oversized responses, malformed results, cross-owner or cross-token-subject access, insecure non-loopback transport, or missing operator configuration. Proposal submission reloads the local immutable bundle, re-verifies the base/candidate hashes and validation verdict, and submits only server-generated canonical JSON, never model-supplied raw definitions. The current observation slice does not yet include arbitrary application logs, user behavior, or business KPI feeds. Replay, shadow, Canary, release, and rollback remain future CloudMold integrations. DeerFlow cannot activate, release, execute, approve, or roll back a workflow; those decisions remain with CloudMold's Workflow Registry, Agent Control, validation, approval, and release gates.
