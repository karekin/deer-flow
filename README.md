...
The local E1 draft plane accepts an active definition only with a fresh CloudMold registry attestation. Configure `CLOUDMOLD_WORKFLOW_REGISTRY_BASE_URL` (an HTTPS URL such as `https://cloudmold.example/admin-api`), a least-privilege `CLOUDMOLD_WORKFLOW_REGISTRY_TOKEN`, its exact DeerFlow-side delegate in `CLOUDMOLD_WORKFLOW_REGISTRY_DEERFLOW_USER_ID`, its expected CloudMold login subject in `CLOUDMOLD_WORKFLOW_REGISTRY_TOKEN_SUBJECT`, and the shared verifier secret `CLOUDMOLD_WORKFLOW_ATTESTATION_SECRET` in DeerFlow; configure the same attestation secret in the CloudMold registry signer. DeerFlow verifies the token's actual server-authenticated CloudMold subject before every business read, so the two user-ID namespaces need not have the same value. Optional `CLOUDMOLD_WORKFLOW_REGISTRY_TIMEOUT_SECONDS` is bounded to 60 seconds. Plain HTTP is rejected except for an explicit loopback-only development opt-in with `CLOUDMOLD_WORKFLOW_ALLOW_INSECURE_LOOPBACK=true`. The environment-token bridge is deliberately one-to-one and single-subject; multi-user deployments must supply per-user delegated tokens rather than a shared privileged token. Secrets are never exposed as agent tool inputs. Imports are bound to the workflow, owner, version, definition hash, and issue time, while proposal risk is constrained by a server-derived minimum.

The Agents workspace can install the managed `workflow-steward` template without hand-editing agent YAML or workflow JSON. The template uses `workflow_definition_get` to retrieve and locally re-verify signed active definitions, `workflow_governance_status` / `workflow_validation_start` to observe and request bounded validation state transitions, `workflow_observation_get` plus `workflow_evidence_timeline` / `workflow_evidence_digest` to read fixed tenant-scoped managed-run, model-observation, and evidence-summary endpoints, `workflow_evidence_ingest` / `workflow_problem_report` / `workflow_feedback_report` to append governed evidence tied to a local immutable proposal bundle, `workflow_manage` to create that bundle, and `workflow_proposal_status` / `workflow_proposal_submit` to register it as a CloudMold candidate when the proposal registry API is available; governed web search remains optional. Managed-run reads use the SkillTask `skill_id` namespace, while model-observation reads require the distinct AI Operations `model_workflow_id` namespace. Governance and evidence writes derive idempotency keys, workflow versions, and lineage from CloudMold status plus the local immutable proposal bundle, so the model never invents those fields. Registry reads reject redirects, ambient proxy credentials, unbounded pages, oversized responses, malformed results, cross-owner or cross-token-subject access, insecure non-loopback transport, or missing operator configuration. Proposal submission reloads the local immutable bundle, re-verifies the base/candidate hashes and validation verdict, and submits only server-generated canonical JSON, never model-supplied raw definitions. Replay, shadow, Canary, approval, release, and rollback remain CloudMold-owned decisions. DeerFlow cannot activate, release, execute, approve, or roll back a workflow; those decisions remain with CloudMold's Workflow Registry, Agent Control, validation, approval, and release gates.
...
Installing `workflow-steward` also reconciles tenant-owned daily and weekly `fresh_thread_per_run` schedules. Each run first uses `workflow_governance_list` (or the optional `CLOUDMOLD_WORKFLOW_STEWARD_TARGET_WORKFLOW_IDS` allowlist) to discover real workflow identifiers, then reviews bounded evidence and reports material problems or candidate opportunities. Cron expressions and timezone can be overridden with `CLOUDMOLD_WORKFLOW_STEWARD_DAILY_CRON`, `CLOUDMOLD_WORKFLOW_STEWARD_WEEKLY_CRON`, and `CLOUDMOLD_WORKFLOW_STEWARD_TIMEZONE`; invalid values fall back to safe defaults.
...
CloudMold deployments may place `business-taxonomy.json` beside the public skill packages. Callers holding the audience-limited `DEER_FLOW_SKILL_CATALOG_AUTH_TOKEN` can read the effective runtime projection through `GET /api/skills/business-catalog`, grouped as business unit → business domain → role → Skill, and retrieve an individual public/integration `SKILL.md` through `GET /api/skills/business-catalog/{skill_name}` using the `X-DeerFlow-Skill-Catalog-Token` header. This token is rejected by every other Gateway route. Both responses include SHA-256 digests and `ETag` headers. The detail endpoint rejects host-local absolute paths and deliberately excludes custom and legacy skill content; CloudMold remains responsible for tenant authorization before forwarding this read-only view to its operations console.
