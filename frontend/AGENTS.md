...
- Managed agent templates are loaded from `/api/agent-templates`; the Agents header offers one-click installation only while a template is not installed. Custom-agent chat links pass both `thread_id` and `assistant_id` to scheduled-task creation so proactive runs retain the managed agent identity.
