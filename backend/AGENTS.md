...
`workflow_definition_get`, `workflow_observation_get`, `workflow_proposal_status`, and `workflow_proposal_submit` are managed-Steward-only CloudMold registry clients. They require an operator-configured HTTPS base URL, bearer token, exact DeerFlow delegate, and expected CloudMold login subject; before every business read or candidate submission they query the protected registry subject endpoint and require the token's server-authenticated subject to match. This explicit one-to-one mapping avoids assuming DeerFlow and CloudMold user IDs share a namespace. The clients disable ambient proxy inheritance and redirects, cap timeouts/responses/pages, unwrap only successful Yudao `CommonResult` payloads, and never accept an agent-selected URL. Plain HTTP requires both a loopback hostname and the explicit local-development flag `CLOUDMOLD_WORKFLOW_ALLOW_INSECURE_LOOPBACK=true`. Multi-user runtimes must resolve per-user delegated tokens. Definition reads locally re-run the same owner/workflow/version/hash/freshness/HMAC verification required by `workflow_manage`; observation reads are currently limited to fixed managed-run (`skill_id`) and model-invocation (`model_workflow_id`) endpoints, whose identifier namespaces must not be mixed. Proposal submission reloads the local immutable bundle from `workflow_manage(create_proposal)`, re-verifies the base/candidate hashes plus local validation verdict, reads the current CloudMold pointer version when needed, and submits only that server-rechecked bundle; it still cannot approve, release, activate, execute, or roll back a workflow. Configure `CLOUDMOLD_WORKFLOW_REGISTRY_BASE_URL`, `CLOUDMOLD_WORKFLOW_REGISTRY_TOKEN`, `CLOUDMOLD_WORKFLOW_REGISTRY_DEERFLOW_USER_ID`, `CLOUDMOLD_WORKFLOW_REGISTRY_TOKEN_SUBJECT`, `CLOUDMOLD_WORKFLOW_ATTESTATION_SECRET`, and optionally `CLOUDMOLD_WORKFLOW_REGISTRY_TIMEOUT_SECONDS`.
