...
def merge_run_metadata(
    ...
    """Merge thread and request metadata while keeping acceptance runs sticky."""
    merged: dict[str, Any] = {}
...
async def start_run(
    ...
    persisted_thread_metadata: Mapping[str, Any] | None = None
    try:
        existing_thread = await run_ctx.thread_store.get(thread_id)
        if isinstance(existing_thread, Mapping) and isinstance(existing_thread.get("metadata"), Mapping):
            persisted_thread_metadata = existing_thread["metadata"]
    except Exception:
        logger.warning(
            "Failed to read persisted metadata for %s; continuing with request metadata",
            sanitize_log_param(thread_id),
            exc_info=True,
        )
